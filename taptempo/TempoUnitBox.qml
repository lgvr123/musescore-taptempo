...
RowLayout {
    ...
    // returned values
    property real unitDuration: -1 // default to -1, so that setting it to 1 will force the onChaged
    readonly property var unitText: (lstMult.currentIndex>=0)?mMultipliers.get(lstMult.currentIndex).sym:undefined
    readonly property var unitFractionNum: (lstMult.currentIndex>=0)?mMultipliers.get(lstMult.currentIndex).fnum:undefined
    readonly property var unitFractionDenum: (lstMult.currentIndex>=0)?mMultipliers.get(lstMult.currentIndex).fdenum:undefined

    onUnitDurationChanged: {
        //the unitDuration value changed from an external property set => we propagate this to the ComboBox
            console.log(">>> setting lstMult currentIndex for multiplier="+unitDuration);
    ...
    // inner data
    default property alias multipliers: mMultipliers 
    ...
    ComboBox {
        ...
        delegate: ItemDelegate {
            ...
            highlighted: mMultipliers.highlightedIndex === index
    ...
    function indexForUnitDuration(ud) {
        ...
        for( var i = 0; i < mMultipliers.rowCount(); i++ ) {
            if (mMultipliers.get(i).mult===ud) {
                console.log("UNIT DURATION: FOUND as "+mMultipliers.get(i).text+", setting index to "+i);
                return i;
            }
        }
